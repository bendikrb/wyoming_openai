...
async def main():
    ...
    stt_client = await CustomAsyncOpenAI.autodetect_backend(
        api_key=args.stt_openai_key, base_url=args.stt_openai_url
    )
    _LOGGER.debug("Detected STT backend: %s", stt_client.backend)

    tts_client = await CustomAsyncOpenAI.autodetect_backend(
        api_key=args.tts_openai_key, base_url=args.tts_openai_url
    )
    ...
    if args.tts_voices:
        ...
        tts_voices = create_tts_voices(
            args.tts_models, args.tts_voices, args.tts_openai_url, args.languages
        )
    else:
        # Otherwise, list supported voices via defaults
        tts_voices = await tts_client.list_supported_voices(
            args.tts_models, args.languages
        )
